...
spark = SparkSession.builder.appName("customer-risk-assessment").getOrCreate()
...
redisServerEncodedDF = spark.sql("select key, zSetEntries[0].element as encodedCustomer from RedisSortedSet")
...
redisServerDecodedDF = redisServerEncodedDF.withColumn("customer", unbase64(redisServerEncodedDF.encodedCustomer).cast("string"))
# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
redisServerDecodedDF.withColumn("customer", from_json("customer", customerSchema))\
    .select(col("customer.*"))\
...
emailAndBirthYear = emailAndBirthDayStreamingDF.select("email", split(emailAndBirthDayStreamingDF.birthDay, "-").getItem(0).alias("birthYear"))
...
emailAndRiskScore = spark.sql("select customer, score from CustomerRisk")
# TO-DO: join the streaming dataframes on the email address to get the risk score and the birth year in the same dataframe
riskScoreByBirthYear = emailAndRiskScore.join(emailAndBirthYear, expr("""
    customer = customerrecords.email
""")
)
...
riskScoreByBirthYear.selectExpr("CAST(customer AS string)", "to_json(struct(*)) AS value")\
    .writeStream\
    .format("kafka")\
    .option("kafka.bootstrap.servers", "kafka:19092")\
    .option("topic", "customer-assessment-risk")\
    .option("checkpointLocation", "/tmp/checkpointlocation1")\
    .start()\
    .awaitTermination()